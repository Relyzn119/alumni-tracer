...
<head>
    ...
    <title>Lupa Password - SIKAK UMI</title>
...
<body class="bg-ground text-snow font-archivo antialiased min-h-screen flex flex-col justify-between relative overflow-x-hidden selection:bg-action selection:text-white">
    ...
    <header class="relative z-10 w-full border-b border-line px-4 lg:px-12 py-4 bg-ground/80 backdrop-blur-md">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <a href="{{ route('login') }}" class="font-mono text-xs text-snow/70 hover:text-action transition-colors text-decoration-none flex items-center gap-2">
            ...
            <div class="mb-8">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <h1 class="font-anybody font-black text-2xl sm:text-3xl uppercase text-snow">
                        RESET <span class="text-action">PASSWORD</span>
                    </h1>
                </div>
                <p class="text-xs text-snow/60 font-mono">
                    Masukkan email terdaftar Anda untuk menerima tautan pengaturan ulang password.
                </p>
            </div>

            @if (session('status'))
                <div class="mb-6 p-4 bg-action/20 border border-action text-snow font-mono text-xs">
                    {{ session('status') }}
                </div>
            @endif
